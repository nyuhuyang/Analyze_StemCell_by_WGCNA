...
traitRows = match(Vassena.Samples, traitData$file_names)
...
#
#=====================================================================================
write.csv(geneInfo, file = "Vassena.geneInfo.csv")
...
Danwei<-Vassena_Danwei[,names(Danwei)]
...
cell_names = vector(mode = "list", length = nSets)
for (set in 1:nSets)
{
        cell_names[[set]] = list(data = as.data.frame(Traits[[set]]$data$cell_names))
        names(cell_names[[set]]$data) = "cell_names"
}
...
MET = consensusOrderMEs(addTraitToMEs(consMEsC, cell_names))
