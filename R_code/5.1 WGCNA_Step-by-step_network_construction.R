...
#        5.1.6 Export of networks to external softHanna
#  5.2 Consensus analysis of naive state Stem Cell and Hanna's expression data
...
#write.csv(matrix,"Hanna_dataset.csv")
...
datExpr0 = as.data.frame(t(Hanna))
head(names(datExpr0))
rownames(datExpr0)
...
#
#=====================================================================================
#  replace all datExpr0 with Hanna.Data

sampleTree = hclust(dist(datExpr0), method = "average")
#  Plot the sample tree: Open a graphic output window of size 12 by 9 inches
#  The user should Hannage the dimensions if the window is too large or too small.
#sizeGrWindow(12,9)
...
#replace all HannaSamples with Hanna.Samples


Hanna.Samples = rownames(datExpr)
traitRows = match(Hanna.Samples, traitData$file_names)
...
save(datExpr, datTraits, file = "Hanna-01-dataInput.RData")
...
lnames = load(file = "Hanna-01-dataInput.RData")
...
par(oma=c(2,2,2,2))
cex1 = 0.9
png(file = "Plots/Hanna scale index.png", 
...
save(MEs, moduleLabels, moduleColors, geneTree, file = "Hanna-02-networkConstruction-stepByStep.RData")

#  Extract modules
#module_colors <- setdiff(unique(mergedColors), "grey")
#for (color in module_colors){
#        module=SubGeneNames[which(mergedColors==color)]
#        write.table(module, paste("Hanna.module_",color, ".txt",sep=""), sep="\t", row.names=FALSE, col.names=FALSE,quote=FALSE)
#}
...
lnames = load(file = "Hanna-02-networkConstruction-stepByStep.RData")
lnames
...
png(file = "Plots/Hanna Module-trait relationships.png", 
    width = 1112, height = 807)
labeledHeatmap(Matrix = moduleTraitCor,
               ...
               cex.text = 0.5,
               zlim = c(-1,1),
               main = paste("Hanna Module-trait relationships"))
dev.off()
...
#
#=====================================================================================
write.csv(geneInfo, file = "Hanna.geneInfo.csv")
...
lnames
#  Load network data saved in the second part.
#lnames = load(file = "Hanna-02-networkConstruction-auto.RData")
#  or----------------
lnames = load(file = "Hanna-02-networkConstruction-stepByStep.RData")
...
#  Taking the dissimilarity to a power, say 10, makes the plot more informative by effectively Hannaging 
...
Protocol = as.data.frame(datTraits$Protocol) #can't be stages,authors,notes
names(Protocol) = "Protocol"
#  Add the cell_names to existing module eigengenes
MEs = orderMEs(cbind(MEs, Protocol))
MET = orderMEs(MEs)
...
par(oma = c(2,2,2,2))
par(oma = c(2,2,2,2))
png(file = "Plots/Hanna Eigengene adjacency heatmap cluster.png", 
    width = 1112, height = 807)
plotEigengeneNetworks(MET, "Hanna Eigengene adjacency heatmap", 
                      marDendro = c(0,4,1,2), 
...
par(cex = 1.0)
plotEigengeneNetworks(MET, "Hanna Eigengene dendrogram",
                      marDendro = c(0,4,2,0),
                      plotHeatmaps = FALSE)
...
par(mar=c(2,2,2,2))
par(oma=c(2,2,2,2))
png(file = "Plots/Hanna Eigengene adjacency heatmap.png", 
    width = 1112, height = 807)
plotEigengeneNetworks(MET, "Hanna Eigengene adjacency heatmap",
