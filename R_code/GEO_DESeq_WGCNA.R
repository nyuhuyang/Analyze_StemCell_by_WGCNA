...
############################################################################
# 
#  2. overall similarity using Spearman correlation
# 
# =====================================================================================
...
source("https://bioconductor.org/biocLite.R")
list.of.bio.packages <- c("AnnotationDbi", "impute", "org.Mm.eg.db","GO.db", "preprocessCore")
new.packages <- list.of.bio.packages[!(list.of.bio.packages %in% installed.packages()[,"Package"])]
if(length(new.packages)) biocLite(new.packages)
...
net = blockwiseModules(datExpr, power = 6,
                       ...
                       verbose = 3)
#We have chosen the soft thresholding power 6,
#a relatively large minimum module size of 30, and a medium sensitivity
#(deepSplit=2) to cluster splitting. The parameter mergeCutHeight is the threshold for
#merging of modules. We have also instructed the function to return numeric, 
#rather than color, labels for modules, and to save the Topological
...
# Isolate Protocol from the clinical traits
Protocol = as.data.frame(datTraits$Protocol_g);
names(Protocol) = "Protocol"
# Add the Protocol to existing module eigengenes
MET = orderMEs(cbind(MEs, Protocol))
